...
async def resolve_instant_link(url: str) -> str | None:
    ...
    link is generated by JavaScript (e.g., 'instant' links).
    """
    logger.info(f"Attempting to resolve instant link: {url}")
    browser = None
    try:
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True, args=['--no-sandbox'])
            ...
            # This selector waits for an <a> tag with id='vd' that also has an
            # href attribute starting with 'https'. This is robust.
            selector = 'a#vd[href^="https"]'
            await page.wait_for_selector(selector, timeout=20000)
            
            download_element = await page.query_selector(selector)
            final_link = await download_element.get_attribute('href')
            
            if final_link:
                logger.info(f"Successfully resolved instant link: {final_link}")
                return final_link
            else:
                logger.warning("Could not extract href from instant link element.")
                return None
    except Exception as e:
        logger.error(f"Error resolving instant link from {url}: {e}")
        return None
    finally:
        if browser and browser.is_connected():
            await browser.close()
...
async def instant_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        _, instant_key = query.data.split(':', 1)
        cached_info = RESULTS_CACHE.get(instant_key)
        if not cached_info:
            await query.edit_message_text("Error: This link has expired. Please try the search again.")
            return
        start_url = cached_info['url']
        ...
        await query.edit_message_text("Error: Invalid instant link data. Please try again.")
        return

    await query.edit_message_text(f"⚡️ Resolving Instant Link... (This may take up to 30s)")
    final_link = await resolve_instant_link(start_url)
    ...
    if final_link:
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text("✅ Success! Your instant link is resolved:", reply_markup=reply_markup)
    else:
        keyboard = [
            [InlineKeyboardButton("📄 Manual Fallback Link", url=start_url)],
            [InlineKeyboardButton("« Back", callback_data=back_callback)]
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text("❌ Failed to resolve the instant link. You can try opening it manually.", reply_markup=reply_markup)
